...
export default class StadiumListItem extends React.Component {
    shouldComponentUpdate(nextProps) {
        return nextProps.stadium !== this.props.stadium;
    }

    render() {
        let {name} = this.props.stadium;
        return (
            <TouchableNativeFeedback
              ...
              backgroundColor:
                Platform.OS === 'android' ? 'transparent' : '#eee',
                        ...
                        <Image
                            resizeMode="contain"
                            source={{ uri: this._getUrlImageLogo()}}
                            style={styles.logo}
                        />
                    </FadeIn>
                </View>
    ...
    _getUrlImageLogo() {
        const {name} = this.props.stadium;
        return dummyName(name, styles.logo)
    }

    _renderHoursText() {
        let {isOpen, openingTimeToday, closingTimeToday} = this.props.stadium;
    ...
    _renderAddressText() {
        let {address, state} = this.props.stadium;

        let addressText = address;

        if (state) {
            addressText = `${addressText} - ${state.join(', ')}`;
        }
