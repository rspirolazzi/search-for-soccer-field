import transformStadium from '../transformStadium';

describe('hours', () => {
  it('correctly calculates when open', () => {
    const openDateTime = new Date("Fri Oct 21 2016 15:55:00 GMT-0700 (PDT)");
    const stadium = transformStadium(exampleStadium, openDateTime);
    expect(stadium.isOpen).toBe(true);
  });

  it('correctly calculates when closed', () => {
    const closedDateTime = new Date("Fri Oct 21 2016 23:55:00 GMT-0700 (PDT)");
    const stadium = transformStadium(exampleStadium, closedDateTime);
    expect(stadium.isOpen).toBe(false);
  });
});
